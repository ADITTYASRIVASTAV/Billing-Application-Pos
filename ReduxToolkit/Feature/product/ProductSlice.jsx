...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    setSearchQuery: (state, action) => {
      ...
      // Only do local filtering if we're not loading from API
      if (!state.loading) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getProductsByStore.rejected, (state, action) => {
        state.loading = false;
        state.products = dummyProducts;
        state.filteredProducts = dummyProducts;
        state.usingDummyData = true;
        state.error = action.payload;
      })
      ...
      .addCase(searchProducts.fulfilled, (state, action) => {
        ...
        // ✅ CRITICAL FIX: Update products array with search results
        const normalizedResults = action.payload.map(item => normalizeProduct(item));
        state.products = normalizedResults;
        state.searchResults = normalizedResults; // Keep this for reference if needed
        ...
        state.usingDummyData = false;
        console.log('Search completed. Products updated:', state.products.length);
      })
...
//     price: product.sellingPrice || product.price || 0,
//     image: product.img || product.image || '',
//     quantity: product.quantity || 0,  // FIXED: Single line for quantity
//     description: product.description || '',
//     mrp: product.mrp || 0,
//     sellingPrice: product.sellingPrice || 0,
//     brand: product.brand || '',
