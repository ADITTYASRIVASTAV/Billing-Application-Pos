  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      if (existingItem) {
        ...
        if (existingItem.cartQuantity < existingItem.availableStock) {
        ...
        if (product.quantity > 0) {
          state.items.push({
            ...
            availableStock: product.quantity, // Total available stock
            cartQuantity: 1, // Quantity in cart
    ...
    updateQuantity: (state, action) => {
      ...
      if (item && quantity > 0 && quantity <= item.availableStock) {
...
export const selectSubtotal = (state) =>
  state.cart.items.reduce((total, item) => total + (item.price * item.cartQuantity), 0);
